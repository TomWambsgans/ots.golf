...
The contract charges `⌈(k + 192) / 512⌉` units for a hash on `k` input bits. A chain hash (128 bits)
still costs one unit, but a ternary grouping hash (384 bits) now costs two and the message index
...
branching factors up to 41, optional digest chains) finds nothing below 109 units, and the flat
...
5. **Charges** (`Potentials.lean`), per unit of query cost:
   ...
   * encoding query after signing: `ε` (index equals `i`).
   Total `≤ 2ε` per unit, so `Pr[forge] ≤ 2ε N = (B-831)/2^127 < B/2^127`.
