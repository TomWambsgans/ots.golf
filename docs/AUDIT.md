...
| Theorem: max_i C_i ≥ 25 | `VerificationLowerBound paperParams 25` | proved by the lower baseline (re-tuned for the two-unit index) |
...
- **F7 (closed).** `VerificationLowerBound paperParams c` would be vacuous if no scheme
  ...
  flat scheme of 41 chains of length 20 verifies at 109 units under the cost model with overhead.
