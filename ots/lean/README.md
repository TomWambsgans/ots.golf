...
bound in `optimal_ots/looking-for-optimal-OTS.tex`: with a 128-bit public key, signatures of at
...
`OptimalOTS/Statement.lean` is the complete statement, in one file and generic in the numerical
...
together with the parameters `paperParams` of the paper. `OptimalOTS/Main.lean` proves
...
The files in `OptimalOTS/Proof/` follow the paper.
