...
theorem initialState_workspace_zero (image : Riscv.Image) (pk : PublicKey paperParams)
    (m : Message paperParams) (bits : List Bool) (hdata : image.data.length ≤ 1048576)
    (addr : Word) (ha : 4195040 ≤ addr.toNat) :
...
/-- The nonce is the first 256 raw signature bits, padded with zeros when absent. -/
theorem initialState_nonce (image : Riscv.Image) (pk : PublicKey paperParams)
    (m : Message paperParams) (bits : List Bool) (hdata : image.data.length ≤ 1048576) :
    MemBits (Riscv.initialState image pk m bits) Riscv.signatureBase
      (ofBits 256 (bits.take 256)) := by
  have h := memBits_extract (start := 0) (len := 256)
  ...
  have heq : ofBits 256 (bits.take 256) = ofBits 256 bits := by
    simpa only [List.drop_zero] using
      ofBits_drop_take bits (cap := 256) (start := 0) (len := 256) (by decide)
  rw [heq]
...
theorem initialState_payload (image : Riscv.Image) (pk : PublicKey paperParams)
    ...
    MemBits (Riscv.initialState image pk m bits)
      (Riscv.signatureBase + BitVec.ofNat 64 (32 + 16 * slot))
      (ofBits 128 (bits.drop (256 + 128 * slot))) := by
  have h := memBits_extract (start := 256 + 128 * slot) (len := 128)
  ...
  have hoff : (256 + 128 * slot) / 8 = 32 + 16 * slot := by omega
