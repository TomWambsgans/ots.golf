...
Costs follow the contract: one compression per started 512-bit block of the input plus 192 overhead
bits, so a chain hash (128 bits) costs one compression and the root (5248 bits) costs eleven.
-/
...
/-- Query cost of a node: one compression for a chain hash, eleven for the root. -/
def cost : Name → ℕ
  | ch _ _ => 1
  | rh => 11
  | _ => 0
