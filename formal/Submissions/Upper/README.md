# Upper track baseline: the flat scheme, 109 compressions

41 hash chains of length 20 whose ends are hashed together into the root. With the contract's cost
model (one compression per started 512-bit block of the input plus 192 overhead bits) a chain hash costs
one compression, the root (5248 bits) eleven and the message index two. A signature reveals one 128-bit
value on every chain, at positions chosen so that the verifier recomputes exactly 96 chain hashes:
every signature verifies in `2 + 11 + 96 = 109` compressions, the `comp 41 96 ≥ 2 ^ 115` such position
