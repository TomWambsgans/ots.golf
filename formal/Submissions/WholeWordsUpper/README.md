...
forest in the order of the `upper` track. A chain step and a grouping hash cost one compression
...
the costs are those of the `upper` track: cuts of reconstruction cost 105 with at most 41
...
Exports (`Solution.lean`): `OptimalOTS.Challenge.WholeWordsUpper.scheme`, `wholeWords`, `secure`,
`cost`.

## Changes from the `upper` track

The proof is the one of `formal/Submissions/Upper` (see `docs/upper-bound-proof.md`), copied and
adapted:
...
```sh
python3 verifier/check_submission.py whole-words-upper
python3 verifier/verify.py whole-words-upper --source .
```
