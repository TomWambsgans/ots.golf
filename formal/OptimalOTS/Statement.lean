...
Every secure scheme is weakly secure (`Scheme.Secure.weaklySecure`), and the lower-bound attacks
forge on a new message, so the lower bounds also hold for weakly secure schemes.
