...
\begin{abstract}
A hash-based one-time signature reveals selected values of a fixed computation on secret inputs, and the verifier recomputes the final hash from them and compares it with the public key. We ask how cheap this verification can be, counting one compression per 512-bit block hashed. With a 128-bit public key, signatures of at most 5504 bits and 127-bit security, some signature must cost at least 25 compressions to verify, and a concrete scheme verifies every signature in 106. Both results are proved in Lean.
\end{abstract}
...
Three quantities compete in such a scheme: the length of the public key, the length of the signature, and the amount of hashing that verification requires. This note fixes the first two and asks for the least possible value of the third. We work in the random-oracle model and charge one \emph{compression} for every 512 bits of input hashed, so that a short input costs one compression and a long input costs proportionally more. Deterministic operations, such as splitting, concatenating, or otherwise transforming strings, are free, and the computation may use them without restriction. Our parameters are a 128-bit public key, signatures of at most 5504 bits, and a security requirement of roughly 127 bits: a classical attacker spending $B$ compressions must forge with probability below $B/2^{127}$.

\paragraph{Result.}
Under these constraints, some signature requires at least 25 compressions to verify (Theorem~\ref{thm:main}). One of these compressions pays for hashing the message together with a nonce, which selects the values to reveal; the remaining 24 are recomputation. In the other direction, Section~\ref{sec:scheme} gives a concrete scheme, a forest of hash chains under a small tree, whose signatures all verify in 106 compressions, and proves it secure (Theorem~\ref{thm:scheme}).

\paragraph{Proof idea.}
Suppose instead that every signature verifies within 24 compressions. Then recomputing the root from any signature evaluates at most 22 hash nodes besides the root. We exhibit an attacker that requests one signature and forges another.
...
\]
compressions. The label does not contribute to the cost. All other computation, storage, and private randomness are free, for both the signer and the attacker.

Deployed schemes hash more than $u$: a public parameter chosen per key, against multi-target attacks, and a tweak that separates the positions of the scheme. Neither is charged here. The parameter can be padded to a full block and absorbed first; the chaining state after that block depends only on the key, so signer and verifier compute it once and start every query from it, and the parameter costs one compression per key instead of one block per query. We owe this observation to Justin Drake. Tweaks are the role of the labels, which separate the queries of the model for free; a scheme that wants them inside its inputs can build them with deterministic nodes and pay for their bits. The idealization shows at block boundaries: a query on three 128-bit values costs one compression here, which is what a hash such as BLAKE2s achieves with a 64-bit tweak, whereas the length padding of SHA-256 pushes the same query into a second block.
...
For a 256-bit message $m$ and a 256-bit nonce $\eta$, let $\mathsf{idx}(m,\eta)$ be the integer represented by the first 128 bits of $H(\tau_{\mathrm{enc}},m\Vert\eta)$, where $\Vert$ denotes concatenation. The label $\tau_{\mathrm{enc}}$ is reserved for this query and differs from every hash-node label. Computing $\mathsf{idx}$ costs one compression.
...
Suppose $C_i\le24$ for every index $i$. After the one-compression index query, root reconstruction costs at most $v=23$ compressions. Every hash node costs at least one compression, and the root belongs to every $E_i$. Hence $|V_i|\le22$, where $V_i=E_i\setminus\{r\}$.
...
\]
A cut in $D$ has between 15 and 41 nodes, so a signature reveals between 2176 and 5248 bits and has at most 5504 bits. Every signature verifies in
\[
...
\]
Every query costs at least one compression. On every path, key generation costs 912 compressions and signing at least one more, so the attacker and the verifier make at most $B-913$ queries. This proves the bound. For $B\le2^{233}$ it gives $\Pr[\mathrm{Forge}]<B/2^{127}$, and for larger $B$ the requirement~\eqref{eq:security} holds because its right-hand side exceeds one.
\end{proof}

\subsection{Machine-checked proof}\label{sec:formal}

Theorem~\ref{thm:scheme} is formalized in Lean~4 on top of the model of Section~\ref{sec:model} (the same \texttt{Scheme} structure, forgery experiment, and cost bound as the formal statement of Theorem~\ref{thm:main}, in \texttt{lean/OptimalOTS/Statement.lean}). The scheme is \texttt{Forest.forestScheme}, with the $128$-bit values of this section as separate truncation nodes, the inputs of the grouping hashes as separate concatenation nodes, and the disclosure sets drawn injectively from the three shapes listed above; the theorems
\begin{center}
\texttt{forestScheme\_verifyCost : forestScheme.verifyCost i = 106},\qquad
\texttt{forestScheme\_secure : forestScheme.Secure}
\end{center}
are proved without \texttt{sorry}, \texttt{native\_decide}, or additional axioms. The formal argument follows this section with two adjustments that suit a proof assistant. First, instead of bounding each event by an expected number of queries, it uses a potential: a function of the oracle cache whose average increase at every fresh query is at most $2^{-127}$ per compression of cost, so that the pathwise budget $B$ of the model directly bounds the probability of the bad events (the counting terms $|V|/M$ and $L\,|W|/(2^{256}-L)$ of the encoding analysis are part of the potential). Second, the independence of hidden inputs from the attacker's view is obtained by resampling one record coordinate below the cut, which leaves the public key, the revealed values and the exposed oracle entries unchanged. The resulting bound is $\Pr[\mathrm{Forge}]\le(B-912)/2^{127}$ for every $B\le2^{127}$, and the requirement~\eqref{eq:security} is trivial for larger $B$.
