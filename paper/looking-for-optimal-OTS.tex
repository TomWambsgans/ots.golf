...
Theorem~\ref{thm:scheme} is formalized in Lean~4 on top of the model of Section~\ref{sec:model} (the same \texttt{Scheme} structure, forgery experiment, and cost bound as the formal statement of Theorem~\ref{thm:main}, in \texttt{formal/OptimalOTS/Statement.lean}, including the 192 overhead bits of the cost). The scheme is \texttt{Forest.forestScheme}, with the $128$-bit values of this section as separate truncation nodes, the input of the root as a separate concatenation node, and the disclosure sets drawn injectively from $D$; the theorems
\begin{center}
\texttt{forestScheme\_verifyCost : forestScheme.verifyCost i = 109},\qquad
\texttt{forestScheme\_secure : forestScheme.Secure}
\end{center}
