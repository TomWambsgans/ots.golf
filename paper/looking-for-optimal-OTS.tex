...
Three quantities compete in such a scheme: the length of the public key, the length of the signature, and the amount of hashing that verification requires. This note fixes the first two and asks for the least possible value of the third. We work in the random-oracle model and charge one \emph{hash unit} for every started 512-bit block of input hashed, so that a short input costs one unit and a long input costs proportionally more. Every query also carries 192 bits of overhead, a 128-bit public parameter for multi-user domain separation and a 64-bit tweak for addressing, as deployed tweakable hash functions do; they are charged but play no role in the model, whose labels provide the separation for free. Deterministic operations, such as splitting, concatenating, or otherwise transforming strings, are free, and the computation may use them without restriction. Our parameters are a 128-bit public key, signatures of at most 5504 bits, and a security requirement of roughly 127 bits: a classical attacker spending $B$ hash units must forge with probability below $B/2^{127}$.

\paragraph{Result.}
Under these constraints, some signature requires at least 25 hash units to verify (Theorem~\ref{thm:main}). Two of these units pay for hashing the message together with a nonce, which selects the values to reveal; the remaining 23 are recomputation. In the other direction, Section~\ref{sec:scheme} gives a concrete scheme, 41 hash chains hashed together into a root, whose signatures all verify in 109 units, and proves it secure (Theorem~\ref{thm:scheme}).

\paragraph{Proof idea.}
Suppose instead that every signature verifies within 24 units. Then recomputing the root from any signature evaluates at most 21 hash nodes besides the root. We exhibit an attacker that requests one signature and forges another.
...
For a 256-bit message $m$ and a 256-bit nonce $\eta$, let $\mathsf{idx}(m,\eta)$ be the integer represented by the first 128 bits of $H(\tau_{\mathrm{enc}},m\Vert\eta)$, where $\Vert$ denotes concatenation. The label $\tau_{\mathrm{enc}}$ is reserved for this query and differs from every hash-node label. Computing $\mathsf{idx}$ costs two units.
...
Suppose $C_i\le24$ for every index $i$. After the two-unit index query, root reconstruction costs at most $v=22$ units. Every hash node costs at least one unit, and the root belongs to every $E_i$. Hence $|V_i|\le21$, where $V_i=E_i\setminus\{r\}$.
...
B=K+2L+2T+(q+2)v+4.
\]
The terms $K$, $2L$ and $2T$ pay for key generation, signing trials, and the nonce search, every index query costing two units. The construction attempts cost at most $qv$, since each checks a subset of the hash nodes used in one reconstruction. Reconstructing from the received values and verifying the final signature cost another $2v$ units for graph queries. Their two index queries cost four further units.
...
This section describes a concrete scheme within the model of Section~\ref{sec:model}. Every signature verifies in 109 units, and the scheme meets the security requirement~\eqref{eq:security}.
...
Figure~\ref{fig:scheme} shows the shape of the computation and of one signature. A chain hash absorbs 128 bits and costs one unit; the root absorbs $41\cdot128=5248$ bits and costs eleven.
...
\[
41\cdot20+11=831\le1024\ \text{units}.
\]
...
Theorem~\ref{thm:main} leaves a gap between 25 and 109 units. The following heuristic suggests that most of this gap cannot be closed by schemes of the usual kind. It is not a proof, and the gap remains open: it rests on an assumption about how forgeries arise that the model of Section~\ref{sec:model} does not impose. Suppose that forging at index $j$ from a signature at index $i\ne j$ always requires the output of a hash node revealed at $i$ and evaluated at $j$, that is, $A_i\cap E_j\ne\emptyset$, while $A_i\cap E_i=\emptyset$. Bollob\'as's set-pair inequality gives $\sum_i\binom{|A_i|+|E_i|}{|A_i|}^{-1}\le1$. A signature reveals at most 41 hash values of 128 bits, so $2^{115}$ disclosure sets force $|E_i|\ge93$ for some $i$, since $\binom{41+92}{41}<2^{115}$. The scheme above evaluates 97 hash nodes and costs 109 units, because the root absorbs 5248 bits and the index query 512.

Among all forests of chains of one length under up to four levels of grouping, with branching factors up to 41 and optional chains of digests above the grouping nodes, an exhaustive computer search within the key-generation budget found nothing below 109 units. With the overhead of 192 bits per query, a grouping hash of three 128-bit values already costs two units, so grouping levels no longer pay for themselves and the flat scheme is optimal in this family; the forest of 63 chains under 21 and 7 digests that is optimal without the overhead costs 125 units here.
...
\]
Every query costs at least one unit. On every path, key generation costs 831 units, so the attacker and the verifier make at most $B-831$ queries. This proves the bound. For $B\le2^{233}$ it gives $\Pr[\mathrm{Forge}]<B/2^{127}$, and for larger $B$ the requirement~\eqref{eq:security} holds because its right-hand side exceeds one.
\end{proof}
