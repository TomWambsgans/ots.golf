    ...
    def test_rename_source_outside_root_is_rejected(self):
        with patch('app.github.httpx.Client') as client:
            ...
            response.json.return_value = [{'filename': 'formal/Submissions/Lower/Solution.lean',
                                           'previous_filename': 'formal/OptimalOTS/Statement.lean'}]
            slug, outside = github.pr_track('owner/repo', 7, expected_files=1)
            self.assertEqual(slug, 'lower')
            self.assertEqual(outside, ['formal/OptimalOTS/Statement.lean'])
    ...
    def test_commit_queue_requires_exact_sha_and_rejects_duplicates(self):
        ...
        for sha in ['a' * 7, 'a' * 39, 'a' * 41, 'x' * 40]:
            with self.assertRaises(HTTPException):
                main.queue_submission(self.session, user, 'lower', 'local', sha, None, [], None, None, None)
        main.queue_submission(self.session, user, 'lower', 'local', 'a' * 40, None, [], None, None, None)
    ...
    def test_simultaneous_admission_cannot_bypass_per_user_limit(self):
        ...
        def submit(n):
            with sessions() as session:
                ...
                try:
                    main.queue_submission(session, user, 'lower', 'local', f'{n:040x}', None, [], None, None, None)
                    return 200
