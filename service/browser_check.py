...
def assert_rules_have_no_scores(text: str, config: dict) -> None:
    ...
    legitimate = {0, 1, 2, 3, 4, 21, 41, 127, 128, 256, 512, 1024, 5248, 5504}
...
def audit(browser: Marionette, base_url: str, output: Path, config: dict) -> None:
    ...
    assert js('return document.querySelector("#framework-disclosure h3").textContent.trim() === "Generality 1/3";')
    ...
    assert js('return document.querySelectorAll(".rules-diagram svg[role=img]").length === 3;')
